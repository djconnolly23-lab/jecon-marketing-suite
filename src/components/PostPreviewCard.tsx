...
export const PostPreviewCard: React.FC<PostPreviewCardProps> = ({
  ...
  const getPlatformBadge = (platform: Platform) => {
    switch (platform) {
      ...
      case 'linkedin': return { bg: 'bg-teal-50 text-teal-800 border-teal-200', dot: 'bg-teal-700', name: 'LinkedIn' };
      ...
      <div className="p-3 bg-slate-50 border-b border-slate-200 flex items-center justify-between text-xs">
        ...
        <div className="flex items-center gap-2">
          <span className={`px-2 py-0.5 rounded text-[11px] font-semibold ${
            ...
            post.status === 'scheduled' ? 'bg-teal-50 text-teal-700 border border-teal-200' :
        ...
        {(post.platform === 'instagram' || post.platform === 'tiktok') && post.aspectRatio === '9:16' ? (
          <div className="bg-slate-900 text-white rounded-lg p-4 relative overflow-hidden flex flex-col justify-between min-h-[290px] border border-slate-800">
            ...
            <div className="my-5 text-center px-3">
              <div className="inline-block p-3 rounded-lg bg-black/60 backdrop-blur-xs border border-white/10 max-w-xs">
                <p className="text-xs font-bold text-teal-200 tracking-tight leading-snug">
                  "{post.hook}"
                </p>
            ...
            <div className="space-y-1.5 z-10">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold">@dako_official</span>
              </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded bg-teal-700 text-white font-bold flex items-center justify-center text-xs">
                J
              </div>
              <div>
                <h4 className="text-xs font-bold text-slate-900 leading-tight">DAKO LLC</h4>
            ...
            <div className="text-xs text-slate-800 space-y-2 leading-relaxed whitespace-pre-line font-sans">
              ...
              {post.bodyCopy.length > 170 && (
                <button
                  ...
                  className="text-teal-700 hover:text-teal-900 font-semibold text-xs inline-block"
                >
                  {expanded ? 'Show less' : '...see more'}
                </button>
              )}
              <p className="text-teal-900 font-medium">{post.callToAction}</p>
              <div className="flex flex-wrap gap-1 text-teal-700 font-medium text-[11px]">
            ...
            <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
              <span className="flex items-center gap-1">
                <ThumbsUp className="w-3.5 h-3.5 text-teal-600" /> 382 Reactions
              </span>
            ...
            <>
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-white bg-[#0284c7] hover:bg-teal-700 rounded-md shadow-xs transition-colors"
                >
